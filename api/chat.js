...
function statusHint(status, seatsLeft, isLeader) {
  ...
  // 완료 상태에서 "이제 출발하세요" 같은 미래형 안내가 나온 적이 있다 — 프롬프트에 경로가
  // 같이 들어가니 모델이 아직 안 탄 걸로 착각한 것이다. 앞으로 할 일이 없다는 걸 명시한다.
  if (status === 'done') return '이미 이동도 정산도 모두 끝난 팟이다. 앞으로 할 일이 없으니, 앞으로의 행동을 안내하지 말고 마무리 인사만 해라.';
  return '';
}
...
const SUMMARY_SYSTEM_PROMPT = '너는 "🤖 택시팟 매니저"라는 채팅봇이야. 경로/하차지점/요금 같은 숫자는 아래에 표로 이미 표시돼 있으니 ' +
  ...
  '"지금 할 일"에 적힌 범위를 벗어나는 행동을 지어내지 마 — 특히 이미 끝난 팟에 앞으로의 행동을 안내하면 안 된다. ' +
