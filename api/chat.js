...
// - summary: "택시팟 매니저" 페르소나로 짧은 인사/안내 한 줄만 준다. 경로/하차지점/요금 숫자는 여기서
// 문장으로 안 내려준다 — 클라이언트가 calcFinalRouteAndFare로 이미 정확히 계산한 값을 표(participant
...
// summary 모드는 숫자를 문장으로 나열하지 않는다 — 그건 클라이언트가 이미 정확히 계산해서
// 화면에 표(participant 카드)로 직접 그린다. 여기선 그 표 위에 붙는 한두 문장짜리 인사/안내만
// 받는다. 숫자를 아예 프롬프트에 안 주면(총액/인원수 정도만 예외) 모델이 잘못된 숫자를 문장에
// 섞어 쓸 걱정 자체가 없다.
...
const SYSTEM_PROMPTS = {
  ...
  summary: '너는 "🤖 택시팟 매니저"라는 채팅봇이야. 아래에 경로/하차지점/요금이 표로 따로 표시될 거라, ' +
...
module.exports = async (req, res) => {
  ...
  const maxTokens = mode === 'summary' ? 60 : 120;
